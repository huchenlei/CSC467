...
int main(int argc, char* argv[]) {
    ...
    if (inputFile != DEFAULT_INPUT_FILE) fclose(inputFile);
    if (errorFile != DEFAULT_ERROR_FILE) fclose(errorFile);
    if (dumpFile != DEFAULT_DUMP_FILE) fclose(dumpFile);
    if (traceFile != DEFAULT_TRACE_FILE) fclose(traceFile);
    if (outputFile != DEFAULT_OUTPUT_FILE) fclose(outputFile);
    if (runInputFile != DEFAULT_RUN_INPUT_FILE) fclose(runInputFile);
            ...
            switch (optarg[1]) {
                ...
                case 'O': /* Alternative output file */
                    ...
                    if (optarg[2] == 0) {
                        i += 1;
                        outputFile =
                            fileOpen(argstr[i], "w", DEFAULT_OUTPUT_FILE);
                        printf("%s\n", argstr[i]);
                    } else
                        outputFile =
                            fileOpen(&optarg[2], "w", DEFAULT_OUTPUT_FILE);
                    printf("%s\n", &optarg[2]);
                    break;
                case 'E': /* Alternative error message file */
                    if (optarg[2] == 0) {
                        ...
                        errorFile =
                            fileOpen(&optarg[2], "w", DEFAULT_ERROR_FILE);
                    break;
                case 'R': /* Alternative sink for traces */
                    if (optarg[2] == 0) {
                        i += 1;
                        traceFile =
                            fileOpen(argstr[i], "w", DEFAULT_TRACE_FILE);
                    } else
                        traceFile =
                            fileOpen(&optarg[2], "w", DEFAULT_TRACE_FILE);
                    break;
                ...
                case 'I': /* Alternative input during execution */
                    if (optarg[2] == 0) {
                        i += 1;
                        runInputFile =
                            fileOpen(argstr[i], "r", DEFAULT_RUN_INPUT_FILE);
                    } else
                        runInputFile =
                            fileOpen(&optarg[2], "r", DEFAULT_RUN_INPUT_FILE);
                    break;
